...
p = [6,6,6,6,6,6,6,6,6,60];
n= sum(p);
tic
% [dis,H,Qi,compare,Q1,Q2,Gs]=testRun(p);
toc
    Q1 = eye(n);
    Q2 = specialOrtho(n);
%     Q1 = specialOrtho(n)
...
% [M,I] = min(dis);
% [M,InMax]= max(dis);
[G1Bar,G2Bar,U,V,theta] =barMatrixAngles(eye(n),Q,p);
G = G2Bar'*G1Bar;
% HImp = H(:,:,I);
% GImp = Gs(:,:,I);
% dmin = dis(I);
...
function [dis,tangents,Qi,compare,Q1,Q2,Gs] = testRun(p)
    n = sum(p);
    d = length(p);
