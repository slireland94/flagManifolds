...
% this script is trying to replace the iterative algorithm that Ma
% introduced. We are going to try use circles with principle angles to try
% do it. Take a few test out.
...
% Here is where we are going to test some things about the circular motion
% using the principle angles out.
...
n = sum(p);
cosDiag = diag(cos(theta));
sinDiag = diag(sin(theta));
check = G2Bar'*(G1Bar)*sinDiag + G2Bar'*G1Bar*cosDiag
check'*check
