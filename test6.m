...
% 1.26.2022
% Issues in this test: When using Gram-Shmidt, your basis could have a
% minus direction. Your vector that you are choosing is random. This means
% that when you do it twice, you could get the other direction. This is an
% interesting problem so I thought I would keep it for now.
...
G2 = Grassmannian(p);
tic
[G,A,G1Bar,G2Bar]=testRun(p,G1,G2);
toc;
tic
[GAlt,AAlt,G1BarAlt,G2BarAlt] = AltTestRun(p,G1,G2);
toc;

% testRun(p)


function [G,A,G1Bar,G2Bar] = testRun(p,G1,G2)
    n = sum(p);
...
function [G,A,G1Bar,G2Bar] = AltTestRun(p,G1,G2)
    n = sum(p);
    % get our points in the Grassmannian (just representatives right now)
    % Thicken the bar of each grassmannian
    % respectively. I.e multiply them by their
