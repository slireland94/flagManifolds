...
tic
[dis,H,Qi,compare,Q1,Q2,Gs,vdis]=testRun(p);
toc
...
[G1Bar,G2Bar,U,V,theta] =barMatrixAngles(Q1,Q2,p);
G = G2Bar'*G1Bar;
tangent = logm(G);
[Hsvd,Gsvd,count] = computeHG(G,p);
examine = expm(Hsvd)*expm(Gsvd);
examine2 = expm(Hsvd);
lexamine = logm(examine);
lexamine2 = logm(examine2);
endG1 = G1Bar*expm(Gsvd);
endG2 = G2Bar;
A = sqrtm(expm(Gsvd));
% B = sqrtm(Gsvd);
casecheck = expm(Gsvd);
...
r = length(p) -1;
m = sum(p(1:r));
v1 = sum(abs(examine(1:m,2)));
v2 = sum(abs(examine(1:m,3)));
v = v1+v2
w1 = sum(abs(examine2(1:m,2)));
w2 = sum(abs(examine2(1:m,3)));
w = w1+w2

a1 = sum(abs(examine(1:n,2)));
a2 = sum(abs(examine(1:n,3)));
a = a1+a2
b1 = sum(abs(examine2(1:n,2)));
b2 = sum(abs(examine2(1:n,3)));
b = b1+b2

c1 = sum(abs(examine(1:m,2)))+sum(abs(examine(2:3,1)));
c2 = sum(abs(examine(1:m,3)));
c = c1+c2
d1 =  sum(abs(examine2(1:m,2)))+sum(abs(examine2(2:3,1)));
d2 = sum(abs(examine2(1:m,3)));
d = d1+d2
examinecheck = abs(examine(3,3)/examine(2,2))-abs(examine2(3,3)/examine2(2,2))
...
dmin = dis(I);
dmax = dis(InMax);
dH = sqrt(.5*trace(Hsvd'*Hsvd));

ad = sqrt(.5*theta*theta');
test1 =dH/ad;
difference = dH - dmin;
diffdis = dmax-dmin;
horizontalDis = sqrt(.5*trace(Hsvd'*Hsvd))
verticalDis = sqrt(.5*trace(Gsvd'*Gsvd))
origDis = sqrt(.5*trace(tangent'*tangent))
toc
...
function [dis,tangents,Qi,compare,Q1,Q2,Gs,vdis] = testRun(p)
    n = sum(p);
    d = length(p);
...
function [G1Bar,G2Bar,U,V,theta] =barMatrixAngles(G1Bar,G2Bar,p)
    l = length(p);
    pAlt =altSyntax(p);
    n = sum(p);
    % We need to add the bar to the first section in a different manner
    ...
    for i =1:(l-1)
        ...
        G1Bar(:,(pAlt(i)+1):(pAlt(i+1))) = G1Slice*Uc;
        G2Bar(:,(pAlt(i)+1):(pAlt(i+1))) = G2Slice*Vc;
        theta((pAlt(i)+1):(pAlt(i+1))) = diag(S);
    end
    U = G1'*G1Bar;
    V = G2'*G2Bar;
    for i=1:p(l)
